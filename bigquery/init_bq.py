#!/usr/bin/env python3
"""
BigQuery Initialization Script for Pub/Sub to BigQuery CDC Demo.

This script:
1. Creates the BigQuery dataset if it doesn't exist
2. Creates the table with PRIMARY KEY (id) NOT ENFORCED for Storage Write API CDC
"""
...
def create_dataset_if_not_exists(client, project_id, dataset_id, location):
    ...
    try:
        ...
        print(f"[INFO] Dataset '{dataset_id}' not found. Creating in {location}...")

        dataset = bigquery.Dataset(dataset_ref)
        dataset.location = location
        dataset.description = "Pub/Sub to BigQuery CDC Demo Dataset"
...
def create_table_if_not_exists(client, project_id, dataset_id, table_id):
    ...
    1. A PRIMARY KEY constraint on the table (NOT ENFORCED)
    2. The Storage Write API to be used for writes
    3. RowMutationInformation for UPSERT and DELETE operations
    """
    ...
    try:
        ...
        CLUSTER BY id
        OPTIONS (
            description = 'Pub/Sub CDC Target Table with PRIMARY KEY for in-place UPSERT/DELETE mutation'
        )
        """
...
def main():
    ...
    print("="*60)
    print(f"  Full table path: {project_id}.{dataset_id}.{table_id}\n")
